...
test('initial value of currentChar is first character "f"', () => {
  ...
  expect(String.fromCodePoint(source.currentChar)).toBe("f");
});
...
test('value of currentChar is "o" after one call to advance()', () => {
  const source = new Source("foo\nbar");
  source.advance();
  expect(String.fromCodePoint(source.currentChar)).toBe("o");
});
...
test('value of currentChar is "\\n" after three calls to advance()', () => {
  ...
  source.advance();
  source.advance();
  expect(String.fromCodePoint(source.currentChar)).toBe("\n");
});
...
test('value of currentChar is "b" after four calls to advance()', () => {
  ...
  source.advance();
  source.advance();
  expect(String.fromCodePoint(source.currentChar)).toBe("b");
});
...
test('value of currentChar is "r" after six calls to advance()', () => {
  ...
  source.advance();
  source.advance();
  expect(String.fromCodePoint(source.currentChar)).toBe("r");
});
